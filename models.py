from sqlalchemy import Column, Integer, String, Identity, Date, ForeignKey, Table
...
reservation_services = Table(
    ...
    Base.metadata,
    Column('rs_id', Integer, Identity(start=1, cycle=False), primary_key=True, index=True),
    Column('r_id', Integer, ForeignKey('reservation.r_id')),
    Column('as_id', Integer, ForeignKey('additional_services.as_id'))
)
...
class Reservation(Base):
    ...
    as_id = Column(Integer, ForeignKey("additional_services.as_id"), nullable=True)  
    r_datearrival = Column(Date)  
    r_datedeparture = Column(Date)  
    r_st = Column(String(50))
    services = relationship("AdditionalService", secondary=reservation_services)
